...
**Core Responsibility**:  
To consume node data from Kafka at scale and persist it idempotently into PostgreSQL, providing a reliable foundation for all downstream processing.
...
Instead of using standard JDBC batch inserts, we leverage PostgreSQL’s native `COPY` command:

1. **Staged CSV Construction** – Node payloads are buffered into in‑memory CSV strings.
2. **Bulk Load** – A single `COPY` operation ingests thousands of rows in milliseconds.
3. **Idempotency** – `INSERT ... ON CONFLICT DO UPDATE` guarantees exactly‑once semantics for node updates.

This reduces round‑trips to the DB from *O(N)* to *O(1)* per batch, enabling ingestion rates exceeding **50k nodes/second**.

---

## **3. Module 3: Perfect Matches Module**

**Core Responsibility**:  
To execute graph‑matching algorithms on candidate pairs and produce optimal, final matches.
...
- **Rationale**  
  By creating a `MatchingStrategySelector`, we make the system extensible and intelligent. Adding a new algorithm is as simple as implementing the `MatchingStrategy` interface and adding a rule to the selector. This avoids a one‑size‑fits‑all approach that would be inefficient or incorrect for certain use cases.

- **Example**  
  Using the `Hungarian` algorithm on a 100,000‑node graph would run for days. The selector correctly routes this to the `Auction` or `Hopcroft‑Karp` algorithm, ensuring the job completes in a reasonable timeframe.

---

### **3.2. Memory‑Aware Processing**

Executing graph algorithms on large datasets is extremely memory‑intensive.

- **Mechanism**  
  The service is configured with a maximum memory budget (e.g., `1024MB`). Before processing a large batch, it checks:
  ```java
  long used = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();
  ```  

- **Action**  
  If current usage exceeds a threshold (e.g., 80% of the budget), the service will either:
    1. Reduce the size of the next processing sub‑batch.
    2. Gracefully fail the job with a clear **“Memory limit exceeded”** error.

- **Rationale**  
  This proactive monitoring prevents the JVM from throwing an `OutOfMemoryError`, which is an unrecoverable state. It ensures the service fails cleanly and predictably.
...
**Core Responsibility**:  
To reliably export the full set of matches (both potential and perfect) and notify the client.
...
1. **Decoupling** – Separates *reading* data from the database from *writing* data to a file/Kafka. The database streaming can run at full speed while the file I/O or Kafka producer handles its own latency.
2. **Parallelism** – Allows I/O operations (reading from DB, writing to file) to happen concurrently, maximizing throughput.
...
4. **Consumer Loop Condition** – The consumer’s loop is:
   ```java
   while (!done.get() || !queue.isEmpty()) 
   ```  
   This elegant condition means: *“Keep running as long as the producers are not done, OR as long as there is still data in the queue to process.”*
