...
public class PerfectMatchStreamingService {
    ...
    public void streamAllMatches(
            UUID groupId,
            UUID domainId,
            Consumer<List<PerfectMatchEntity>> batchConsumer,
            int batchSize
    ) {
        ...
        while (retryCount <= maxRetries) {
            try (Connection conn = dataSource.getConnection();
                 PreparedStatement ps = conn.prepareStatement(
                         QueryUtils.getAllPerfectMatchesStreamingSQL(),
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    List<PerfectMatchEntity> buffer = new ArrayList<>(batchSize);
                    int totalRecords = 0;
                    ...
                    while (rs.next()) {
                        ...
                        PerfectMatchEntity entity = new PerfectMatchEntity();
                ...
                if (retryCount > maxRetries) {
                    throw new RuntimeException("Error streaming perfect matches after " + maxRetries + " retries", e);
                }
